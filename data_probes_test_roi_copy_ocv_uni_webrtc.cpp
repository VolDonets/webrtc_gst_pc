//
// Created by biba_bo on 2020-08-05.
//
...
#define WIDTH 1280
#define HEIGHT 720
#define STR_WIDTH "1280"
#define STR_HEIGHT "720"
...
static GstPadProbeReturn cb_have_data (GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
    ...
    if (gst_buffer_map (buffer, &map, GST_MAP_WRITE)) {
        //*
        cv::Mat head_image, head_image_1, head_image_2;
        cv::Size frame_size(WIDTH, HEIGHT);

        head_image = cv::Mat(frame_size, CV_8UC4, (char*)(map.data), cv::Mat::AUTO_STEP);
        head_image_1 = head_image.clone();
        head_image_2 = head_image.clone();

        cv::Rect my_interest_region(50, 350, 300, 200);

        cv::Mat cropped_img = head_image_1(my_interest_region);

        cv::Mat main_image, mini_image;
        cv::resize(cropped_img, main_image, cv::Size(WIDTH,HEIGHT));
        cv::resize(head_image_2, mini_image, cv::Size(MINI_WIDTH, MINI_HEIGHT));

        cv::Rect mini_insertion_coord(450, 50, mini_image.cols, mini_image.rows);
        cv::Rect main_insertion_coord(0, 0, head_image.cols, head_image.rows);

        //std::cout << "rows::: " << main_image.rows << " cols::: " << main_image.cols << "\n";

        main_image.copyTo(head_image(main_insertion_coord));
        mini_image.copyTo(head_image(mini_insertion_coord));

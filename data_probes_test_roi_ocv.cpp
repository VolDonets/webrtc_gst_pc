...
static GstPadProbeReturn cb_have_data (GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
    ...
    if (gst_buffer_map (buffer, &map, GST_MAP_WRITE)) {
        ...
        cv::rectangle(frame_image, my_interest_region, cv::Scalar(255), 6, 8, 0);

        cv::Mat cropped_img = frame_image(my_interest_region);
        cv::Rect cropped_insertion_coord(450, 50, cropped_img.cols, cropped_img.rows);
        cropped_img.copyTo(frame_image(cropped_insertion_coord));

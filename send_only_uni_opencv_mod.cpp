//
// Created by biba_bo on 2020-07-27.
//
...
#define STR_WIDTH "800"
#define STR_HEIGHT "600"
...
create_receiver_entry (SoupWebsocketConnection * connection)
{
    ...
    /*
    GstElement *pipeline, *video_src, *videorate, *video_filter,
                *video_convert, *video_convert_filter, *video_convert_last, *queue,
                *x264enc, *x264enc_filter, *queue_second, *h264parse, *rtph264pay,
                *rtp264py_filter, *webrtcbin;
    */
    GstElement *pipeline_0, *pipeline_1;
    pipeline_0 = gst_parse_launch("v4l2src device=/dev/video0 "
                                  "! videorate "
    ...
    /*/receiver_entry->pipeline =
                gst_parse_launch ("webrtcbin name=webrtcbin  stun-server=stun://" STUN_SERVER " "
                                  ...
                                  "! videorate "
                                  "! video/x-raw,width=" STR_WIDTH ",height=" STR_HEIGHT ",framerate=15/1 "
                                  "! videoconvert name=ocv_video_src "
                                  "! video/x-raw,format=BGRA "
                                  "! videoconvert "
                                  "! queue max-size-buffers=1 "
                                  "! x264enc speed-preset=ultrafast tune=zerolatency key-int-max=15 "
    ...
    gst_element_set_state(receiver_entry->pipeline_0, GST_STATE_PLAYING);
...
destroy_receiver_entry (gpointer receiver_entry_ptr)
{
    ...
    if (receiver_entry->pipeline != NULL) {
        if (receiver_entry->pipeline_0 != NULL)
            gst_element_set_state(GST_ELEMENT(receiver_entry->pipeline_0), GST_STATE_NULL);
        gst_element_set_state (GST_ELEMENT (receiver_entry->pipeline),
                               GST_STATE_NULL);

        gst_object_unref (GST_OBJECT (receiver_entry->webrtcbin));
        if (receiver_entry->pipeline_0 != NULL)
            gst_object_unref (GST_OBJECT (receiver_entry->pipeline_0));
